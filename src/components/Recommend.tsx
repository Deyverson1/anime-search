...
function Recommend() {
  return (
    <section className="px-2 py-8 ">
      <h1 className="pb-4 pl-4 text-xl font-bold text-black lg:pl-4">Recommended:</h1>
      <section className="flex flex-wrap justify-center flex-1 gap-y-8 lg:gap-y-16 gap-x-1 lg:gap-x-4">
        {
          dataBase.map(({ title, image, subTitle, id }) => (
            <Link key={title} to={`/anime/${id}`}>
              <article className="flex flex-col flex-1 space-x-0 space-y-2 md:gap-x-10 lg:space-y-1 group md:space-y-0 lg:h-80">
                ...
                <div>
                  <p className="font-bold text-md text-gray-950">{title}</p>
