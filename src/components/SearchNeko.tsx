        ...
        {Array.isArray(response) && response !== null && response.length > 0 && response.map((data: {image: string, artistName: string, artist: string, liked: string}, index) => (
          <article key={index} className="rounded-lg " style={{ backgroundColor: '' }}>
            ...
            <div className="px-2 text-center">
              <h1 className="pb-2 text-lg font-bold text-gray-800 dark:text-gray-300">{data.artistName}</h1>
              <div>
                <a target="blank" href={data.artist}>
                  <button type="button" className="text-white bg-gray-600 hover:bg-blue-400 focus:ring-4 focus:outline-none focus:ring-[#050708]/50 font-medium rounded-lg text-sm px-5 py-2 text-center inline-flex items-center dark:focus:ring-[#050708]/50 dark:hover:bg-blue-400 me-2 mb-2 gap-x-2">
