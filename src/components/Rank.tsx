  ...
  return (
    <section className="flex items-center rounded-md mb-8 p-2 gap-x-8 border-white border-2">
      <div className="flex flex-col items-center justify-center px-4 pr-6 border-r-2">
        ...
        {score && (<p className="flex gap-x-2 items-center"><strong>{score}</strong><Star /> </p>)}
        {scored_by && (<p className="text-sm text-center">{scored_by.toLocaleString('en-US')} users</p>)}
      </div>
      <div className="flex gap-x-2 gap-y-2 flex-wrap">
        ...
        {favorites && (<p className="bg-red-600 px-3 text-md rounded-sm">Favorties {favorites.toLocaleString('en-US')}</p>)}
      </div>
    </section>
  )
}