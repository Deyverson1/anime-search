...
interface AnimeResultsProps {
  data: { imageUrl: string, title: string }[] | null,
}
  ...
  return (
    <section className="px-1 lg:pt-20 lg:px-40">
      ...
      <main className=" flex  justify-center w-full gap-y-4 gap-x-2 lg:gap-x-4 lg:gap-y-4  flex-wrap flex-1">
        {data !== null && data.map((dato: { imageUrl: string, title: string }, index: number) => (
          <article key={index} className="flex flex-col flex-1 md:gap-x-10  space-x-0 lg:space-y-8 group md:space-y-0 lg:h-72 max-w-36">
