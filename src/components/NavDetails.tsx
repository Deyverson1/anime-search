...
export default function NavDetails() {
  ...
  return (
    <section className="flex flex-col items-center justify-between w-full lg:flex-row">
      <section className="top-0 flex items-center justify-between w-full h-16 px-8 py-10 bg-[#1d1e20] bg-cover lg:px0 lg:px-40 lg:items-stretch ">
        <Link to="/" className="flex">
          <div className={`flex items-center justify-center gap-x-2 lg:w-full ${!isSearch ? '' : 'hidden'}`}>
        ...
        <header className={`flex justify-between lg:justify-end lg:px-4 w-full lg:w-10/12 transition-opacity duration-1000 ${!isSearch ? '' : 'hidden'}`}>
          ...
          <Link to="/" className="text-sm uppercase text-white rounded-lg px-4 py-2 bg-[#2f3237] hover:bg-[#43474d] hover:transition-all duration-300 hidden md:block">Home</Link>
          </div>
